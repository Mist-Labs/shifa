...
Latest uploaded detector: YOLO11n, 640px input, TFLite size 5,350,968 bytes. Validation gates on firearm detection: `GUN` mAP50 **0.725** against a 0.60 release target. Overall mAP50 is not the release gate because the current Roboflow split has insufficient knife signal; `KNIFE` remains experimental. Android still-image Guard evidence uses this TFLite artifact through the native mobile bridge before cloud fallback.
